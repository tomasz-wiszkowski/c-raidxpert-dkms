...
module_param(rc_adapter_count, int, S_IRUGO);
...
module_param(rc_suspend_delay, int, S_IRUGO);
...
static void rc_shutdown_host(struct Scsi_Host *host_ptr);
static void rc_shutdown_adapter(rc_adapter_t *adapter);
static void rcraid_shutdown_one(struct pci_dev *pdev);
static int rc_ioctl(struct scsi_device *scsi_dev_ptr, unsigned int cmd,
		    void *arg);

void rc_dump_scp(struct scsi_cmnd *scp);
static const char *rc_info(struct Scsi_Host *host_ptr);
...
static int rc_ahci_init(rc_adapter_t *adapter);
static int rc_ahci_start(rc_adapter_t *adapter);
static int rc_ahci_shutdown(rc_adapter_t *adapter);
static irqreturn_t rc_ahci_isr(int irq, void *arg, struct pt_regs *regs);
#endif // RC_AHCI_SUPPORT

// Additions for NVME driver
static int rc_nvme_init(rc_adapter_t *adapter);
static int rc_nvme_start(rc_adapter_t *adapter);
static int rc_nvme_shutdown(rc_adapter_t *adapter);
static irqreturn_t rc_nvme_isr(int irq, void *arg, struct pt_regs *regs);

static void rc_init_proc(void);
static void rc_remove_proc(void);
...
#ifdef RC_AHCI_SUPPORT
static rc_version_t rc_ahci_version = { .init_func = rc_ahci_init,
...
static rc_version_t rc_nvme_version = { .init_func = rc_nvme_init,
 ...
 *    in 2.6 kernels.
 */
static void rc_init_module(void)
{
	...
	if (get_efi()) {
		...
		efi_guid_t gAmdNvmeTrapDeviceVarGuid =
			NVME_TRAP_DEVICE_VAR_GUID;
 ...
 */
static void rc_get_bar(struct pci_dev *dev, int which_bar,
		       struct rc_pci_bar *result)
{
...
static int rc_init_adapter(struct pci_dev *dev, const struct pci_device_id *id)
{
	...
	// Check for device before calling init_func to ensure RC_EnableZPODD properly set.
	{
		...
		//
		size = sizeof(devStat);
		ac_stat = rc_acpi_evaluate_object(handle, "ODDZ._STA", &devStat,
						  &size);
		if (ACPI_SUCCESS(ac_stat) && (devStat & 0xB) == 0xB) {
			RC_ODD_Device = RC_ODD_DEVICE_ODDZ;

			size = sizeof(devAddr);
			ac_stat = rc_acpi_evaluate_object(handle, "ODDZ._ADR",
							  &devAddr, &size);
			if (ACPI_SUCCESS(ac_stat)) {
				RC_ODDZDevAddr = devAddr;

				size = sizeof(package);
				ac_stat = rc_acpi_evaluate_object(
					handle, "ODDZ._PRW", package, &size);
				if (ACPI_SUCCESS(ac_stat)) {
					union acpi_object *acpi_obj =
						(union acpi_object *)package;
					union acpi_object *aObj =
						&acpi_obj->package.elements[0];

					if (acpi_obj->type ==
						    ACPI_TYPE_PACKAGE &&
					    acpi_obj->package.count == 2) {
						RC_ODD_GpeNumber =
							(unsigned int)aObj
								->integer.value;
					}
			...
			size = sizeof(devStat);
			ac_stat = rc_acpi_evaluate_object(handle, "ODDL._STA",
							  &devStat, &size);
			...
			if (ACPI_SUCCESS(ac_stat) && (devStat & 0xB) == 0xB) {
				...
				size = sizeof(devAddr);
				ac_stat = rc_acpi_evaluate_object(
					handle, "ODDL._ADR", &devAddr, &size);
				if (ACPI_SUCCESS(ac_stat)) {
					RC_ODDZDevAddr = devAddr;

					size = sizeof(package);
					ac_stat = rc_acpi_evaluate_object(
						handle, "ODDL._PRW", package,
						&size);
					if (ACPI_SUCCESS(ac_stat)) {
						union acpi_object *acpi_obj =
							(union acpi_object *)
	...
	if (!RC_EnableZPODD) {
		...
		RC_PRINTK(
			...
			"RC_ODD_GpeNumber = %d\n",
			__FUNCTION__, RC_EnableZPODD,
			ODD_Devices[RC_ODD_Device], RC_ODDZDevAddr,
			RC_ODD_GpeNumber);
	}

	{
		...
		mem = dmam_alloc_coherent(&dev->dev, 1024 * 1024, &mem_dma,
					  GFP_KERNEL);
	}
 ...
 *    parameters.
 */
static int rc_init_host(struct pci_dev *pdev)
{
	int error;
 ...
 */
static int rcraid_probe_one(struct pci_dev *dev, const struct pci_device_id *id)
{
	int ret = -ENODEV;
	...
	if (!adapter_count) {
		...
		for (probe_id = &rcraid_id_tbl[0]; probe_id->vendor != 0;
		     probe_id++) {
			probe_dev = NULL;
			while ((probe_dev = pci_get_subsys(
					probe_id->vendor, probe_id->device,
					probe_id->subvendor,
					probe_id->subdevice, probe_dev))) {
...
static int rcraid_suspend_one(struct pci_dev *pdev, pm_message_t mesg)
{
	...
	for (i = rc_state.num_hba - 1; i > 0; i--) {
		...
		pci_set_power_state(adapter->pdev,
				    pci_choose_state(adapter->pdev, mesg));
...
int rc_nvme_init(rc_adapter_t *adapter)
{
	...
	if (adapter->pdev->msix_cap) {
		...
		if (pci_enable_msix_exact(adapter->pdev,
					  &adapter->hardware.msix_ent, 1)) {
			// Non-zero - failed
			dev_err(&adapter->pdev->dev,
				"rc_nvme_init: pci_enable_msix_exact failed\n");
		} else {
			// Zero - succeeded
		...
		if (pci_enable_msi(adapter->pdev)) {
			dev_err(&adapter->pdev->dev,
				"rc_nvme_init: pci_enable_msi failed\n");
		} else {
			adapter->hardware.ismsi = 1;
		}
	}
 ...
 */
int rc_eh_abort_cmd(struct scsi_cmnd *scp)
{
	rc_srb_t *srb;
	...
	if (srb != NULL) {
		RC_PRINTK(RC_DEBUG,
			  ...
			  srb->flags, srb->bus, srb->target, srb->lun);
...
static ssize_t rc_proc_write_dipm(struct file *file, const char __user *buffer,
...
static int rc_proc_dipm_open(struct inode *inode, struct file *file)
{
...
static ssize_t rc_proc_write_hipm(struct file *file, const char __user *buffer,
				  size_t count, loff_t *off)
{
	int err;
	unsigned long num;
...
static int rc_proc_hipm_open(struct inode *inode, struct file *file)
{
...
static ssize_t rc_proc_write_debug(struct file *file, const char __user *buffer,
				   size_t count, loff_t *off)
{
	int err;
	unsigned long num;
...
static int rc_proc_debug_show(struct seq_file *sfile, void *v)
{
	seq_printf(sfile, "%d %d %d\n", rc_msg_level, RC_PANIC, RC_TAIL - 1);
	return 0;
}

static int rc_proc_debug_open(struct inode *inode, struct file *file)
{
...
static ssize_t rc_proc_write_an(struct file *file, const char __user *buffer,
				size_t count, loff_t *off)
{
	int err;
	unsigned long num;
...
static int rc_proc_an_open(struct inode *inode, struct file *file)
{
...
static ssize_t rc_proc_write_zpodd(struct file *file, const char __user *buffer,
				   size_t count, loff_t *off)
{
	int err;
	unsigned long num;
...
static int rc_proc_zpodd_open(struct inode *inode, struct file *file)
{
...
static ssize_t rc_proc_write_delay(struct file *file, const char __user *buffer,
				   size_t count, loff_t *off)
{
	int err;
	unsigned long num;
...
static int rc_proc_delay_open(struct inode *inode, struct file *file)
{
...
static int rc_proc_version_show(struct seq_file *sfile, void *v)
{
	seq_printf(sfile, "V%s %s\n", RC_DRIVER_VERSION, RC_BUILD_NUMBER);
	return 0;
}

static int rc_proc_version_open(struct inode *inode, struct file *file)
{
...
static int rc_proc_stats_show(struct seq_file *sfile, void *v)
{
	char *kbuf;
...
static int rc_proc_stats_open(struct inode *inode, struct file *file)
{
...
} rc_proc_entries[] = { { "dipm", &rc_proc_dipm_fops },
	...
	if (proc_parent == NULL) {
		...
		if (proc_parent) {
			...
			while (rpe && rpe->name) {
				...
				pde = proc_create(rpe->name, 0, proc_parent,
						  rpe->fops);
				rpe++;
			}
		}
...
static struct ctl_table rcraid_table[] = { { .procname = "dipm",
					     .data = &RC_EnableDIPM,
...
static struct ctl_table rcraid_dir_table[] = {
	{ .procname = "rcraid", .mode = 0555, .child = rcraid_table },
	{}
};
...
static struct ctl_table rcraid_root_table[] = { { .procname = "dev",
						  .mode = 0555,
#if 0
...
static int __init rcraid_init(void)
{
	...
	snprintf(version_string, VERSION_STRING_LEN, "V%s %s",
		 RC_DRIVER_VERSION, RC_BUILD_NUMBER);
