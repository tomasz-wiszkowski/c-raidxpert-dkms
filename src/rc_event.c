...
int rc_event_init(void)
{
	...
	tasklet_init(&tp->cfg_change_detect.tasklet,
	...
	tp->thread =
		kthread_run(rc_event_kthread, (void *)tp, "rc_event_kthread");
...
static int rc_event_kthread(void *rc_threadp)
{
	rc_event_thread_t *tp;
	rc_srb_t *srb;
	rc_softstate_t *state;
	...
	do {
		...
		while (tp->cfg_change_response.head) {
			...
			spin_unlock_irqrestore(&tp->cfg_change_response.lock,
					       irql);
...
static int rc_notify_scsi_layer(int channel, int target, int lun, int present)
{
