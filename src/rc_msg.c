...
static void rc_msg_cpuid(unsigned int function, unsigned int *eax,
 ...
 *
 */
void rc_send_msg(struct rc_send_arg_s *args)
{
	preempt_disable();
...
void rc_msg_pci_config(rc_pci_op_t *pci_op, u32 call_type)
{
	...
	if (pci_op && (pci_op->adapter < MAX_HBA) &&
		...
		case RC_PCI_READ_CONFIG_BYTE:
			pci_op->status = pci_read_config_byte(
		...
		case RC_PCI_READ_CONFIG_WORD:
			pci_op->status = pci_read_config_word(
				pdev, pci_op->offset, &tmp16);
			if (!pci_op->status) {
				pci_op->val = tmp16;
			}
			break;
		case RC_PCI_READ_CONFIG_DWORD:
			pci_op->status = pci_read_config_dword(
		...
		case RC_PCI_WRITE_CONFIG_DWORD:
			pci_op->status = pci_write_config_dword(
				pdev, pci_op->offset, pci_op->val);
			break;
		default:
	...
	while (!kthread_should_stop()) {
		...
		while (acpi_work_item_head) {
			...
			case RC_ACPI_INVOKE:
				...
				if (args->u.acpi.inPtr == NULL &&
				    args->u.acpi.outPtr == NULL &&
				    args->u.acpi.inSize == 0 &&
				    args->u.acpi.outSize == 0) {
					//
					...
					// another hack... Sigh.)
					//
					if (strncmp((char *)&rc_work->method
							    [strlen(rc_work->method) -
							     4],
						    "_PS3", 4) == 0) {
						acpi_status ac_stat;

						// Clear any pending notifcations
						ac_stat = acpi_clear_gpe(
							NULL, RC_ODD_GpeNumber);
						// Arm for notification
						ac_stat = acpi_enable_gpe(
							NULL, RC_ODD_GpeNumber);
					}
					// Invoke only
					args->u.acpi
						.status = acpi_evaluate_object(
						(acpi_handle)rc_work->handle,
						rc_work->method, NULL, NULL);
				} else {
					union acpi_object *out_object;
					struct acpi_buffer outBuf = {
			...
			if (acpi_work_item_head == acpi_work_item_tail) {
				acpi_work_item_head = acpi_work_item_tail =
					NULL;
			} else {
...
void rc_receive_msg(void)
{
	...
	case RC_CTR_EVENT:
		rc_event(args->u.event.rc_notification_type,
			 args->u.event.rc_bus_changed, RC_SRB_GLOBAL_UPDATE);
		break;
	...
	case RC_CTR_PRINT_VA:
		rc_vprintf(args->u.print_va.severity, args->u.print_va.format,
			   args->u.print_va.va_l);
		break;
	...
	case RC_CTR_WAIT_MICROSECONDS:
		...
		if ((state->osic_locked) &&
		    ((delay < 15000) || (state->state & INIT_DONE))) {
			preempt_disable();
			udelay(delay);
			preempt_enable();
		} else {
	...
	case RC_ACPI_INVOKE: {
		...
		if (rc_wq) {
			work = (rc_work_t *)kmalloc(sizeof(rc_work_t),
						    GFP_KERNEL);
			if (work) {
				int ret;
				...
				work->method = (char *)kmalloc(
					strlen(args->u.acpi.method) + 1,
					GFP_KERNEL);
				work->handle = handle;
				work->args = args;
				if (work->method) {
	...
	case RC_ACPI_REGISTER: {
		...
		case RC_ODD_DEVICE_ODDZ:
			args->u.acpi.status = acpi_get_handle(
				NULL, "\\_SB.PCI0.SATA.ODDZ", &handle);
			break;
		case RC_ODD_DEVICE_ODDL:
			args->u.acpi.status = acpi_get_handle(
				NULL, "\\_SB.PCI0.SATA.ODDL", &handle);
			break;
		case RC_ODD_DEVICE_ODD8:
			args->u.acpi.status = acpi_get_handle(
				NULL, "\\_SB.PCI0.SATA.ODD8", &handle);
			break;
		}
...
void rc_msg_cpuid(unsigned int function, unsigned int *eax, unsigned int *ebx,
...
int rc_msg_init(rc_softstate_t *state)
{
	...
	state->memsize_per_controller =
		args.u.get_info.memory_size_per_controller;
	...
	for (i = 0; i < state->num_hba; i++) {
		...
		args.u.init_controller.controller_memory =
		...
		args.u.init_controller.orig_vendor_id =
			adapter->hardware.orig_vendor_id;
	...
	state->cache_memory_size =
		args.u.init_controller.cache_memory_size_needed;
	...
	args.u.final_init.size_of_virtual_memory_allocated =
		state->virtual_memory_size;
	args.u.final_init.size_of_cache_memory_allocated =
		state->cache_memory_size;
...
void rc_msg_srb_done(rc_srb_t *srb)
{
	rc_softstate_t *state;
	// unsigned long irql;
...
void rc_msg_srb_complete(struct rc_srb_s *srb)
{
	...
	if (srb->function == RC_SRB_SHUTDOWN ||
	    srb->function == RC_SRB_RESTART || srb->function == RC_SRB_FLUSH) {
		rc_softstate_t *state;
	...
	if (!(srb->flags & RC_SRB_FLAGS_SENSEVALID)) {
		if (srb->status == RC_SRB_STATUS_INVALID_REQUEST) {
			...
			rc_set_sense_data((char *)scp->sense_buffer,
					  SENKEY_ILLEGAL,
					  SENCODE_INVALID_MESSAGE_ERROR,
					  ASENCODE_INVALID_MESSAGE_ERROR, 0, 0,
					  0, 0);
		} else {
			...
			rc_set_sense_data((char *)scp->sense_buffer,
					  ...
					  ASENCODE_INTERNAL_TARGET_FAILURE, 0,
					  0, 0, 0);
		}
	}
...
void rc_msg_send_srb_function(rc_softstate_t *state, int function)
{
	rc_srb_t *srb;
	rc_send_arg_t args;
	int size;
...
void rc_msg_get_dma_memory(alloc_dma_address_t *dma_address)
{
	...
	dma_address->cpu_addr = dma_alloc_coherent(
		&adapter->pdev->dev, dma_address->bytes, dmaHandle, GFP_ATOMIC);

	if (dma_address->cpu_addr) {
		rc_add_dmaMemoryList(dma_address->cpu_addr, dmaHandle,
				     dma_address->bytes, adapter);
	}
}
...
void rc_msg_free_all_dma_memory(rc_adapter_t *adapter)
{
	...
	while (dmaNode) {
		rc_msg_free_dma_memory(adapter, dmaNode->cpu_addr,
				       dmaNode->dmaHandle, dmaNode->bytes);

		adapter->dmaMemoryListHead =
			adapter->dmaMemoryListHead->nextNode;
		kfree(dmaNode);
		dmaNode = adapter->dmaMemoryListHead;
	}
...
void rc_msg_map_mem(struct map_memory_s *map)
{
	...
	if ((map->memory_id & MEM_TYPE) == RC_MEM_PADDR) {
		...
		if ((vaddr >= private_start) && (vaddr < private_end)) {
			...
			map->physical_address =
				dma_map_page(&adapter->pdev->dev, page, offset,
					     len_mapped, DMA_BIDIRECTIONAL);
...
int rc_msg_stats(char *buf, int buf_size)
{
	...
	len = snprintf(cp, buf_size - cnt,
		       "max SRBs sent %u max intr waiting %u "
